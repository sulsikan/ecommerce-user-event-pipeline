...
- Prometheus is added for operational metrics because Grafana works well with it
  and it cleanly captures Kafka, Spark, and validation signals.
- ClickHouse is added as the business aggregate query store for Grafana.
- Dashboard labels should be readable without external lookup.

## Metrics Inventory

### Business Metrics in ClickHouse
...
| Purchase count | `category_label`, `brand_label` | Sales activity. |
| Revenue | `category_label`, `brand_label` | Revenue trend. |
| Average purchase price | `category_label`, `brand_label` | Basket/product signal. |
...
| `clickhouse_write_latency_seconds` | `table` | Sink health. |
...
| Aggregate stale | critical | freshness > 5m during replay | check Spark-to-ClickHouse sink |
...
| ClickHouse aggregate store | data engineering | check write latency and table health |
